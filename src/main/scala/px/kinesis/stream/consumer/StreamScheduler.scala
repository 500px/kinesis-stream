...
class StreamScheduler(config: ConsumerConfig)(
  ...
  def start(): Future[Done] = startSchedulerAndRegisterShutdown(SchedulerExecutionContext.Global)

  private def startSchedulerAndRegisterShutdown(implicit ec: ExecutionContext): Future[Done] = {
  ...
  private def shutdown(scheduler: Scheduler)(implicit ec: ExecutionContext): Future[Done] = {
  ...
  private def createScheduler(config: ConsumerConfig)(publishSink: Sink[Record, NotUsed], killSwitch: KillSwitch) = {
    val configsBuilder =
      new ConfigsBuilder(
        config.streamName,
        config.appName,
        ...
        config.workerId,
        new RecordProcessorFactoryImpl(publishSink, config.workerId, tracker, killSwitch)(am, ec, logging)
      )
    ...
    val leaseManagementConfig = config.leaseManagementConfig.getOrElse(configsBuilder.leaseManagementConfig())
    val lifecycleConfig = configsBuilder.lifecycleConfig().taskExecutionListener(new ShardShutdownListener(tracker))
...
object StreamScheduler {
  def apply(config: ConsumerConfig)
  (
    publishSink: Sink[Record, NotUsed],
    killSwitch: KillSwitch,
    terminationFuture: Future[Done]
